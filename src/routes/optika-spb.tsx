...
const faq: SeoFaqItem[] = [
  ...
  {
    q: "Можно ли пройти проверку зрения бесплатно?",
    a: "Стоимость проверки зависит от выбранной услуги и действующих условий салона. Возможность бесплатной проверки в рамках акции или заказа очков нужно уточнить при записи.",
  },
  {
    q: "Есть ли рассрочка на очки и линзы?",
    a: "Доступные способы оплаты и условия рассрочки зависят от товара и действующих предложений партнёров. Актуальные условия следует уточнить у консультанта перед оформлением заказа.",
  },
  {
    q: "Как записаться на диагностику зрения?",
    a: "Записаться можно через кнопку записи на этой странице или по телефону 8-800-700-0214. Предварительная запись помогает выбрать удобное время визита.",
  },
  {
...
function OptikaSpbPage() {
  return (
    ...
    >
      ...
      <h2>Как проходит комплексная диагностика</h2>
      <p>
        Приём может включать объективное измерение рефракции и кривизны роговицы, проверку остроты
        зрения вдаль и вблизи, субъективный подбор коррекции и оценку бинокулярного баланса. Перед
        подбором контактных линз специалист также оценивает состояние переднего отрезка глаза и
        посадку выбранной модели. Состав обследования определяется индивидуально.
      </p>

      <h2>Как часто проверять зрение</h2>
      <p>
        Периодичность зависит от возраста, жалоб, способа коррекции и рекомендаций специалиста.
        Детям и людям с меняющейся рефракцией контроль обычно нужен чаще; пользователям контактных
        линз важна регулярная оценка посадки и поверхности глаза. При резком снижении зрения,
        вспышках, двоении или ощущении «занавески» обращаться за медицинской помощью нужно внепланово.
      </p>

      <h2>Детская оптика и контроль миопии</h2>
      <p>
        Для детей доступны оправы, очковые и контактные решения для контроля миопии. План коррекции
        выбирают после обследования и затем оценивают в динамике; подробнее о точном измерении
        осевой длины рассказывает страница <a href="/biometriya-glaza/">биометрия глаза</a>.
      </p>

      <h2>Почему выбирают «Оптика 100%»</h2>
      <p>
        В одном месте можно проверить зрение, подобрать оправу, очковые или контактные линзы,
        оформить изготовление очков и получить рекомендации по дальнейшему наблюдению. Салон
        находится в центре Санкт-Петербурга рядом с метро «Чернышевская».
      </p>
      ...
      <p>
        ...
        странице <a href={PRIMARY_SALON.productionPath}>салона на Кирочной, 17</a>.
      </p>
      <p>
